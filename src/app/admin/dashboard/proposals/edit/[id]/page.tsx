...
export default function EditProposalPage() {
  ...
  const fetchProposal = async () => {
    try {
      const response = await fetch(`/api/admin/proposals/${params.id}`);
      if (response.ok) {
        const data = await response.json();
        setFormData({
          business_name: data.proposal.business_name,
          title: data.proposal.title,
          body: data.proposal.body,
          theme_color: data.proposal.theme_color || '#10B981',
        });
      } else {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Edit Proposal</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Update the drafted message and details</p>
        </div>
      </div>
