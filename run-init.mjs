...
const logFile = path.resolve(process.cwd(), 'db-init.log');
function log(msg) {
  console.log(msg);
...
async function init() {
  try {
    ...
    for (let queryStr of fields) {
      log(`Executing: ${queryStr}`);
      await sql.query(queryStr);
    }

    log("✓ All fields added successfully.");
  } catch (error) {
